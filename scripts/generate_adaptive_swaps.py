...
def load_swaps(path: Path) -> dict[str, tuple[int, list[tuple[str, str, str]]]]:
    data = tomllib.loads(path.read_text())
    layouts: dict[str, tuple[int, list[tuple[str, str, str]]]] = {}
    for layout, config in data.items():
        ...
        layouts[layout] = (timeout, parse_swaps(layout, raw_swaps))
    return layouts
...
def process(path: Path, gen_dir: Path, timeout: int, swaps: list[tuple[str, str, str]]) -> None:
    text = path.read_text()
    ...
    header = f"/* Generated by scripts/generate — do not edit. Source: keymap/adaptive_swaps.toml */"
