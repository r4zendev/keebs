...
/*
 * Bodged Wysteria wired build.
 *
 * This intentionally stays as a separate QMK keymap so the bodged board can be
 * rebuilt with the current generated shared layers without replacing the normal
 * Wysteria target. The recovered April 30 bodged artifact used the same scanned
 * row/column GPIOs as the normal board; the distinction was the QMK keymap name.
 *
 * ZMK source layout: 5+5 / 5+5 / 5+5 / 2+2.
 * Wysteria physical layout: 5+5 / 5+5 / 5+5 / 3+3.
 * Upstream QMK's LAYOUT_wyst_custom still requires two extra bottom-cluster
 * matrix positions for the encoder/inner slots, so this wrapper pads those
 * with KC_NO and keeps the visible thumb mapping to 3+3.
 * Use the tuckie plus the middle lower thumb on each side.
 * Wysteria's physical thumb order in QMK's bottom cluster is:
 *   left:  tuckie, middle, stretchie
 *   right: stretchie, middle, tuckie
 */
#define RAZEN_LAYOUT( \
    ...
    LAYOUT_wyst_custom( \
        ...
        LM4, LM3, LM2, LM1, LM0,       RM0, RM1, RM2, RM3, RM4, \
        LB4, LB3, LB2, LB1, LB0,       RB0, RB1, RB2, RB3, RB4, \
        /* left: tuckie, middle, stretchie, encoder */ \
        LH2, LH1, KC_NO, KC_NO, \
        /* right: encoder, stretchie, middle, tuckie */ \
        KC_NO, KC_NO, RH1, RH2 \
    )
...
enum positions {
    ...
    P_LH2, P_LH1, P_LX0, P_LX1, P_RX0, P_RX1, P_RH1, P_RH2,
};
...
bool oled_task_user(void) {
    if (is_keyboard_master()) {
        ...
        oled_write_ln_P(PSTR("wysteria"), false);
    }
    return false;
}
#endif
